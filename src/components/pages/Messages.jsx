      ...
      if (response.ok) {
        ...
        if (currentUserId && mongoConversations.length > 0) {
          ...
          for (const conversation of mongoConversations) {
            try {
              await messageService.ensureConversationExists(
                conversation._id,
                currentUserId,
              );
            } catch (syncError) {
              console.warn(
                'Failed to sync conversation to Supabase:',
                conversation._id,
                syncError,
              );
            }
  ...
  const fetchSpecificConversation = async (conversationId) => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.conversation) {
          ...
          if (currentUserId) {
            try {
              ...
              console.warn(
                'Failed to sync specific conversation to Supabase:',
                syncError,
              );
            }
          }
        ...
        console.error(
          'Failed to fetch specific conversation:',
          response.status,
        );
        const errorData = await response.json();
        console.error('Error data:', errorData);
