  ...
  return (
    <div className="rounded-lg p-4 w-full bg-accent">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        {canEditOrDelete(post) && (
          <div className="relative">
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-1 w-32 bg-background rounded-md shadow-lg border border-gray-600 z-10">
                <button
                  onClick={() => {
                    setEditingPost(true);
      ...
      {editingPost ? (
        ...
        <div className="mb-3">
          ...
          {post.post_type === 'image' &&
            post.secure_image_urls &&
            post.secure_image_urls.length > 0 && (
              <div className="mt-3">
                <div
                  className={`grid gap-2 rounded-lg overflow-hidden ${
                    post.secure_image_urls.length === 1
                      ? 'grid-cols-1 max-w-md'
                      : post.secure_image_urls.length === 2
                        ? 'grid-cols-2 max-w-2xl'
                        : post.secure_image_urls.length === 3
                          ? 'grid-cols-2 max-w-2xl'
                          : 'grid-cols-2 max-w-2xl'
                  }`}
                >
                  {post.secure_image_urls.map((url, index) => (
                    <div
                      key={index}
                      className={`${
                        post.secure_image_urls.length === 3 && index === 0
                          ? 'col-span-2'
                          : ''
                      }`}
                    >
      ...
      <div className="flex items-center space-x-4 text-sm text-gray-400">
        ...
        >
          <Heart
            className={`w-4 h-4 ${post.is_liked_by_user ? 'fill-current' : ''}`}
          />
          ...
          {post.replies.map((reply) => (
            <ETReply
              ...
              onEdit={(replyId, content) =>
                onEdit(replyId, content, true, post._id)
              }
