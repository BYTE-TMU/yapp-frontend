...
function Waypoint() {
  ...
  const fetchWaypoints = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/waypoint/campus/tmu?radius=2`,
        {
          method: 'GET',
      ...
      const transformedWaypoints = await Promise.all(
        data.waypoints.map(async (waypoint) => {
          ...
          if (waypoint.type === 'event' && currentUserId && eventsData) {
            try {
              ...
              let matchingEvent = eventsData.events?.find((event) => {
                ...
                if (
                  ...
                  const lngDiff = Math.abs(
                    waypoint.longitude - event.longitude,
                  );
              ...
              if (!matchingEvent) {
                console.log(
                  `🔄 No coordinate match for "${eventTitle}", trying title-only...`,
                );
                matchingEvent = eventsData.events?.find((event) => {
                  ...
                  if (titleMatch) {
                    console.log(
                      `✅ Found title-only match for attendance: "${event.title}"`,
                    );
                  }
                  return titleMatch;
                });
              ...
              if (matchingEvent) {
                ...
                if (attendanceResponse.ok) {
                  ...
                  console.warn(
                    `❌ Failed to get attendance status for "${eventTitle}"`,
                  );
                }
              }
            } catch (error) {
              console.warn(
                'Error fetching attendance for waypoint:',
                waypoint.title,
                error,
              );
            }
          ...
          const waypointData = {
            ...
            isLiked: currentUserId
              ? waypoint.liked_users?.includes(currentUserId)
              : false,
            isBookmarked: currentUserId
              ? waypoint.bookmarked_users?.includes(currentUserId)
              : false,
  ...
  const goToPreviousSaved = () => {
    ...
    const newIndex =
      currentSavedIndex > 0 ? currentSavedIndex - 1 : savedWaypoints.length - 1;
  ...
  const goToNextSaved = () => {
    ...
    const newIndex =
      currentSavedIndex < savedWaypoints.length - 1 ? currentSavedIndex + 1 : 0;
    setCurrentSavedIndex(newIndex);
    navigateToSavedWaypoint(savedWaypoints[newIndex]);
  };
  ...
  const joinEventFromWaypoint = async (waypoint) => {
    try {
      ...
      const matchingEvent = events.find((event) => {
        ...
        const titleMatch =
          event.title.toLowerCase().trim() === eventTitle.toLowerCase().trim();
      ...
      if (!matchingEvent) {
        ...
        const partialMatch = events.find((event) => {
          ...
          const partialMatch =
            eventTitleLower.includes(waypointTitleLower) ||
            waypointTitleLower.includes(eventTitleLower);
          return partialMatch;
        });
        ...
        if (partialMatch) {
          ...
          const confirmed = await showEventMatchConfirmation(
            partialMatch.title,
          );
          if (!confirmed) return;
  ...
  const deleteWaypoint = async (waypointId, waypointTitle) => {
    try {
      ...
      const isEventWaypoint =
        waypoint &&
        (waypoint.type === 'event' || waypoint.title.startsWith('📅'));
  ...
  const deleteEventFromWaypoint = async (waypoint, waypointTitle) => {
    try {
      ...
      if (!matchingEvent) {
        console.log(
          '❌ No matching event found, falling back to waypoint deletion',
        );
        ...
        const response = await fetch(
          `${API_BASE_URL}/waypoint/${waypoint.id}`,
          {
            method: 'DELETE',
            headers: getAuthHeaders(),
          },
      ...
      const waypointDeleteResponse = await fetch(
        `${API_BASE_URL}/waypoint/${waypoint.id}`,
        {
          method: 'DELETE',
          headers: getAuthHeaders(),
      ...
      if (!waypointDeleteResponse.ok) {
        console.warn(
          '⚠️ Failed to delete waypoint after cancelling event:',
          waypointDeleteResponse.status,
        );
  ...
  const findAndNavigateToEventWaypoint = (eventData) => {
    ...
    console.log(
      'Event waypoints available:',
      waypoints.filter((w) => w.type === 'event'),
    );
    ...
    const eventWaypoint = waypoints.find((waypoint) => {
      ...
      const waypointTitle = waypoint.title
        .replace(/^📅\s*/, '')
        .trim()
        .toLowerCase();
    ...
    if (eventWaypoint) {
      ...
      showWaypointError(
        'This event waypoint is not currently available on the map. The event waypoint may have expired or was not created.',
      );
    }
  };
