...
const FollowingModal = ({ isOpen, onClose, userId, isOwnProfile }) => {
  ...
  const fetchFollowing = async () => {
    try {
      ...
      if (isOwnProfile) {
        ...
        if (token) {
          try {
            ...
            url = `${API_BASE_URL}/users/me/following`;
            targetUserId = null;
          }
        }
      }
      ...
      if (!url) {
        url = `${API_BASE_URL}/users/${targetUserId}/following`;
      }
  ...
  const handleFollowToggle = async (followingId) => {
    if (actionLoading[followingId]) return;
    
    setActionLoading(prev => ({ ...prev, [followingId]: true }));
    try {
      ...
      // If unfollowed, remove from the following list entirely
      if (!data.following) {
        setFollowing(prev => prev.filter(followingUser => followingUser._id !== followingId));
      } else {
        // Update the following's follow status in the list
        setFollowing(prev => prev.map(followingUser => 
          followingUser._id === followingId 
            ? { ...followingUser, is_following: data.following }
            : followingUser
        ));
      }
      ...
      setActionLoading(prev => ({ ...prev, [followingId]: false }));
    }
  };
  ...
  const scrollbarClass = `following-modal-scrollbar-${isDarkMode ? 'dark' : 'light'}`;
            ...
            {loading ? (
              <div className="flex justify-center items-center h-64">
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <div className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Loading following...</div>
                </div>
              </div>
            ) : error ? (
              <div className="flex justify-center items-center h-64">
              ...
              <div className="space-y-3">
                {following.map((followingUser) => (
                  <div key={followingUser._id} className={`rounded-xl p-4 transition-all duration-200 ${
                    isDarkMode 
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <img
                          src={getProfilePictureUrl(followingUser.profile_picture)}
