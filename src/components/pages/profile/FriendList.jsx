...
const FriendList = ({ userId, isOwnProfile }) => {
  ...
  useEffect(() => {
    fetchFriends();
  }, [userId, isOwnProfile, fetchFriends]);
        ...
        <div className="flex space-x-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex flex-col items-center animate-pulse">
              <div className="w-16 h-16 rounded-full mb-2 bg-muted"></div>
  ...
  if (error) {
    return (
      <div className="rounded-lg p-6 mb-6 border border-border bg-card">
        ...
        <p className="text-destructive text-sm">Failed to load friends</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg p-6 mb-6 border border-border bg-card">
      <div className="flex items-center justify-between mb-4">
        ...
        {friends.length > 7 && (
          <button className="flex items-center text-sm font-bold transition-colors text-primary hover:text-primary/80">
            <span>See all</span>
      ...
      <div className="flex space-x-4 overflow-x-auto pb-2 scrollbar-hide">
        {friends.slice(0, 7).map((friend) => (
          <div
            ...
            className="flex flex-col items-center cursor-pointer hover:opacity-80 transition-opacity shrink-0"
          >
            <div className="relative">
              <img
                src={getProfilePictureUrl(friend)}
                alt={friend.username}
                className="w-16 h-16 rounded-full object-cover border-2 transition-colors border-muted hover:border-muted-foreground"
              />
              {friend.is_verified && (
            ...
            <span className="text-sm font-medium mt-2 text-center max-w-[70px] truncate text-foreground">
              {friend.username}
            </span>
          </div>
        ))}
        ...
        {friends.length > 7 && (
          <div className="flex flex-col items-center cursor-pointer hover:opacity-80 transition-opacity shrink-0">
            <div className="w-16 h-16 rounded-full transition-colors flex items-center justify-center border-2 bg-muted hover:bg-muted/80 border-muted">
              <span className="text-xs font-bold text-foreground">
                +{friends.length - 7}
              </span>
            </div>
            <span className="text-sm font-medium mt-2 text-center text-muted-foreground">
              More
            </span>
          </div>
        )}
