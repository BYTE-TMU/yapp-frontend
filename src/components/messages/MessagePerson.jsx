...
function MessagePerson({ conversation, isSelected, onClick, formatTime }) {
  ...
  const { other_participant, last_message, last_message_at, unread_count } =
    conversation || {};
  ...
  const getCurrentUserIdentifier = () => {
    try {
      ...
      if (userString) {
        ...
        const userId =
          user._id || user.id || user.userId || user.user_id || user.username;
    ...
    <div
      ...
      className={`flex items-center p-3 cursor-pointer transition-all duration-200 relative ${
        ...
        isUnseen && !isSelected // Added !isSelected check here too
          ? isDarkMode
            ? 'bg-linear-to-r from-orange-900/20 to-transparent border-l-4 border-orange-500 shadow-lg'
            : 'bg-linear-to-r from-orange-100 to-transparent border-l-4 border-orange-500 shadow-md'
          : ''
      }`}
      style={{
        minHeight: '60px',
          ...
          {isUnseen && !isSelected && (
            <div className="flex items-center">
              {unread_count && unread_count > 1 ? (
                ...
                <div className="w-3 h-3 bg-orange-500 rounded-full shrink-0 animate-pulse shadow-lg"></div>
              )}
            </div>
          )}
        </div>
