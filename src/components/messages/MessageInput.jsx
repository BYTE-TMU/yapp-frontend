...
function MessageInput({
  ...
  const handleSubmit = async (e) => {
    ...
    if ((!newMessage.trim() && !attachedImage) || sending || disabled) {
      console.log(
        '❌ Cannot send: empty message and no attachment, already sending, or disabled',
      );
      return;
    }
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/messages/upload-attachment`,
        {
          method: 'POST',
          headers: {
      ...
      <form onSubmit={handleSubmit} className="flex items-center space-x-3">
        ...
        <div className="flex-1 relative">
          <input
            ...
            className={`w-full rounded-lg px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-colors border ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={sending || disabled}
            autoComplete="off"
            maxLength={1000}
          />
