...
const MessageBubble = React.memo(function MessageBubble({
  ...
  const loadSecureImageUrl = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/messages/attachment/${encodeURIComponent(message.attachment_s3_key)}`,
        {
          headers: {
  ...
  const bubbleStyles = useMemo(() => {
    let baseStyles =
      'inline-block max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl px-3 py-2 break-words cursor-pointer transition-all duration-200 hover:scale-[1.02] shadow-sm';
  ...
  return (
    <div
      className={`group relative ${isMyMessage ? 'text-right' : 'text-left'} animate-in fade-in-0 slide-in-from-bottom-2 duration-300`}
    >
      <div
      ...
      >
        ...
        (message.attachment_url || imageUrl) ? (
        ...
        {message.edited && (
          <span
            className={`text-xs italic ml-2 opacity-75 ${
              isMyMessage
                ? 'text-orange-200'
                : isDarkMode
                  ? 'text-gray-400'
      ...
      {/* Message metadata */}
      <div
        className={`flex items-center mt-1 px-1 ${
      ...
      {messageStatus === 'failed' && (
        ...
        >
          <span className="text-red-500">Failed to send. Tap to retry.</span>
        </div>
      )}
