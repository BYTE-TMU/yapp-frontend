  ...
  (
    ...
    const isMyMessage = useCallback(
      (message) => {
        ...
        const isMyMsg =
          String(messageSenderId) === String(currentUserIdentifier);
    ...
    const messageGroups = useMemo(() => {
      console.log(
        '🔄 Recalculating message groups for',
        messages.length,
        'messages',
      );
...
const MessageGroup = React.memo(
  ({ group, showDateSeparator, getProfilePictureUrl }) => {
    return (
      <div>
        {/* Date separator */}
        {showDateSeparator && (
        ...
        {/* Message group */}
        <div
          className={`flex ${group.isMyMessage ? 'justify-end' : 'justify-start'} mb-3`}
        >
          <div
            className={`flex ${group.isMyMessage ? 'flex-row-reverse items-end' : 'flex-row items-end'} max-w-[85%] sm:max-w-[75%] md:max-w-[70%]`}
          >
            ...
            {/* Messages in group */}
            <div
              className={`flex flex-col space-y-1 ${group.isMyMessage ? 'items-end' : 'items-start'}`}
            >
            ...
            {group.isMyMessage && <div className="w-10 shrink-0"></div>}
          </div>
        </div>
      </div>
    );
