...
function ResetPasswordForm() {
  ...
  const handleVerifyCode = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/password-reset/verify-reset-code`,
        {
          method: 'POST',
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/password-reset/resend-reset-code`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        {step === 1 ? (
          <>
            ...
            >
              ...
              <input
                type="text"
                className="w-full px-4 py-3 rounded-lg bg-muted text-foreground border border-border focus:border-primary focus:outline-none transition-colors text-center text-xl tracking-widest"
                placeholder="123456"
                value={code}
                onChange={(e) =>
